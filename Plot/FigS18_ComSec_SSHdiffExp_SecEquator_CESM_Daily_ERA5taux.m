...
for one = 1:3 % 1:NTAandTIOandPMMandSEPandWWBbmay;2:NTAandTIOandPMMandSEPandWWBb;3:NTAandTIOandPMMandSEPandWWBb
    ...
    aimpath = ['F:\2023PMM_Work\Figures_for_Publish\Figure\FigS18_SecEquator_Monthly\']
    if exist(aimpath,'dir')~=7
        mkdir(aimpath);
    end
    
    pathfig = [aimpath,'FigS18',first_name3{one},'_Exp_',first_name{one},'_SSHA_Sec',fourth_name{four},third_name{three},fifth_name{five}];
    load([path1,'Compose_SSHdiff',second_name{two},'_Global_Daily_2023-03_to_2024-02_Exp_',first_name{one},name1{1}(end-12:end-8),'.mat']);% SSH
    data1 = load([path1,'SSH_Casely_Daily/SSH_Global_Daily_2023-03_to_2024-02_Exp_',first_name{one},name1{1}(end-12:end-8),'_01.mat']);
    ...
    time = data1.time;
    ssha = ssha_ensamble*100;% cm
    t_ssha = t_ssha_ensamble;
    
    date2 = data2.date;
    time2 = data2.time;
    uvela = data2.uvela_ensamble*100;% cm/s
    ...
    bin_ssha = squeeze(nanmean(ssha(a,b,:),2));
    bin_ssha_t = squeeze(nanmean(t_ssha(a,b,:),2));
    ...
    % cb_RedBlue(13,:) = [1,1,1];
    % cb_RedBlue(14,:) = [1,1,1];
    data_c = load('F:\2023PMM_Work\bin_data\Colorbar_SSH.mat');
    cb_RedBlue = data_c.mycmap;
    len_cb = size(cb_RedBlue,1);
    
    k1 = 6;
    bin1_cb_RedBlue = cb_RedBlue(1:len_cb/2,:);
    bin2_cb_RedBlue = cb_RedBlue(len_cb/2+1:end,:);
    bin1_cb_RedBlue = bin1_cb_RedBlue(1:end-k1,:);
    bin2_cb_RedBlue = bin2_cb_RedBlue(k1+1:end,:);
    cb_RedBlue = cat(1,bin1_cb_RedBlue,bin2_cb_RedBlue);
    
    k2 = 1;
    cb_RedBlue(size(cb_RedBlue,1)/2-k2+1:size(cb_RedBlue,1)/2+k2,:) = ones(2*k2,3);
    %%
    close all
    [cs,h] = contourf(fig1_lon,fig1_time,fig1','levelstep',lv_ssha);
    hold on
    ...
    if one == 1
        ...
        pathfig2 = [aimpath,'Colorbar_FigS18',first_name3{one},'_Exp_',first_name{one},'_SSTA_Sec',fourth_name{four},third_name{three},fifth_name{five}];
        set(gcf,'PaperUnits','inches','PaperPosition',PaperPosition,'color',[1 1 1]);
