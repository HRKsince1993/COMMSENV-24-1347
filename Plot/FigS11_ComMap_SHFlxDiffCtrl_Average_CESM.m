...
for one = 1:1 % 1:PMM;2:SEP;3:TIO;4:NTA;5:NTAandTIOandPMMandSEPlg;6:NTAandTIOandPMMandSEPandWWBb
    ...
    six = 5; % 1:latent heat flux;2:sensible heat flux;3:longwave;4:shortwave;5:All
    ...
    sixth_name = {'SLHFlx','SSHFlx','SLwFlx','SSwFlx','SHFlx'};
    sixth_name2 = {'Latent','Sensible','Longwave','Shortwave','Surface heat flux'};
    sixth_name3 = {'11a','11b','11c','11d','11'};
    sixth_bin = [-1,-1,-1,1,1];
    ...
    % len_m = (11:13)-2;% month
    %%
    aimpath = ['F:\2023PMM_Work\Figures_for_Publish\Figure\FigS',sixth_name3{six},'_ComMap_',sixth_name{six},'DiffCtrl_Average_Mon',num2str(len_m(1)+2),'to',num2str(len_m(end)+2),'\'];
    if exist(aimpath,'dir')~=7
        mkdir(aimpath);
    end

    pathfig = [aimpath,fifth_name{five},'FigS',sixth_name3{six},first_name3{one},'_Com_',sixth_name{six},'Diff',second_name{two},'_Month',num2str(len_m(1)+2),'to',num2str(len_m(end)+2),'_mEnA_Exp_',first_name{one},'_Color',num2str(three)];

    data1a = load([path1,'Compose_',sixth_name{six},'Diff',second_name{two},'_Global_Monthly_2023-03_to_2024-02_Exp_',first_name{one},name1{1}(end-12:end-5),'_Avr',num2str(len_m(1)+2),'to',num2str(len_m(end)+2),'Mon.mat']);% Heat flux
    data1b = load([path1,'Compose_',sixth_name{1},'Diff',second_name{two},'_Global_Monthly_2023-03_to_2024-02_Exp_',first_name{one},name1{1}(end-12:end-5),'_Avr',num2str(len_m(1)+2),'to',num2str(len_m(end)+2),'Mon.mat']);% Heat flux
    ...
    nino34_ndj = mean(mean(mean(data2.ssta_ensamble(a1,b1,9:11))));

    switch six
        case 1
            shflxa = sixth_bin(six) * data1a.slflxa_ensamble;% positive means heat ocean
            t_shflxa = data1a.t_slflxa_ensamble;
        case 2
            shflxa = sixth_bin(six) * data1a.ssflxa_ensamble;% positive means heat ocean
            t_shflxa = data1a.t_ssflxa_ensamble;
        case 3
            shflxa = sixth_bin(six) * data1a.slwflxa_ensamble;% positive means heat ocean
            t_shflxa = data1a.t_slwflxa_ensamble;
        case 4
            shflxa = sixth_bin(six) * data1a.sswflxa_ensamble;% positive means heat ocean
            t_shflxa = data1a.t_sswflxa_ensamble;
        case 5
            shflxa = sixth_bin(six) * data1a.sflxa_ensamble;% positive means heat ocean
            t_shflxa = data1a.t_sflxa_ensamble;
    end
    slflxa = sixth_bin(1) * data1b.slflxa_ensamble;% positive means heat ocean
    t_slflxa = data1b.t_slflxa_ensamble;

    ssta = data3.ssta_ensamble;
    uvela = data4.uvela_ensamble;
    vvela = data4.vvela_ensamble;
    % preci = preci_ensamble*24*3600*1000;% change to mm/day

    t_ssta = data3.t_ssta_ensamble;
    t_uvela = data4.t_uvela_ensamble;
    t_vvela = data4.t_vvela_ensamble;
    ...
    bin_uvela_t = t_uvela;
    bin_vvela_t = t_vvela;
    bin_shflxa_t = t_shflxa;
    bin_slflxa_t = t_slflxa;

    bin_vela_t = bin_uvela_t + bin_vvela_t;
    bin_vela_t( bin_vela_t > 0 ) = 1;

    bin_ssta_t(bin_ssta_t == 0) = nan;
    bin_shflxa_t(bin_shflxa_t == 0) = nan;
    bin_slflxa_t(bin_slflxa_t == 0) = nan;
    ...
    lv_slflx = 5;
    if three == 1
        cb_RedBlue = input_colorbar_rgb('F:\2023PMM_Work\bin_data\colorbar\cb_heatflux.rgb')/255;
    end

    color_plot = [151,137,246 ...% 蓝紫色 NETP
        ;75,0,192 ...% 2蓝紫色 SETP
        ;255,0,255 ... % 3紫红色 TIO
        ;1,132,127 ...;% 4马尔斯绿 TA
        ;255,119,15]/255; % 5爱马仕橙 G
    ...
    bin_fig1 = bin_shflxa(a,b,:).*bin_shflxa_t(a,b,:);
    % bin_fig2 = bin_ssta(a,b,:).*bin_ssta_t(a,b,:);
    bin_fig2 = bin_slflxa(a,b,:).*bin_slflxa_t(a,b,:);
    ...
    [cs,h] = m_contourf(lon,lat,fig1,'levelstep',lv_shflx);

    % d_num = 2;
    % [x2,y2]=find(fig1_test'>=1);
    % m_plot(lon(x2(1:d_num:end)),lat(y2(1:d_num:end)),'g.','markersize',2,'color',[0.3,0.3,0.3]);% plot dots

    [cs2,h2] = m_contour(lon,lat,fig2a,'color',[1,0.27,0],'LineWidth',LW2,'levelstep',lv_slflx);
    [cs3,h3] = m_contour(lon,lat,fig2b,'color',[0.24,0.4,0.9],'LineWidth',LW2,'levelstep',lv_slflx);
    ...
    if one == 1 || one >= 5 % NETP
        ...
        m_plot([lon_point2(1),lon_point1(1)],[lat_point1(1),lat_point1(1)],'color',color_plot(1,:),'LineWidth',LW4,'LineStyle',LineStyle_box)
        m_plot([lon_point1(1),lon_point1(2)],[lat_point1(1),lat_point1(2)],'color',color_plot(1,:),'LineWidth',LW4,'LineStyle',LineStyle_box)
        m_plot([lon_point1(2),lon_point2(2)],[10,10],'color',color_plot(1,:),'LineWidth',LW4,'LineStyle',LineStyle_box)
        m_plot([lon_point2(2),lon_point2(1)],[lat_point2(2),lat_point2(1)],'color',color_plot(1,:),'LineWidth',LW4,'LineStyle',LineStyle_box)
        m_plot([lon_point2(1),lon_point2(1)],[lat_point2(1),lat_point1(1)],'color',color_plot(1,:),'LineWidth',LW4,'LineStyle',LineStyle_box)
    end
    ...
    if one == 2 || one >= 5 % SETP
        ...
        m_plot([lon_point(2),lon_point(1)],[lat_point(2),lat_point(2)],'color',color_plot(2,:),'LineWidth',LW4,'LineStyle',LineStyle_box)
        m_plot([lon_point(1),lon_point(1)],[lat_point(2),lat_point(1)],'color',color_plot(2,:),'LineWidth',LW4,'LineStyle',LineStyle_box)
        m_plot([lon_point(1),lon_point(2)],[lat_point(1),lat_point(1)],'color',color_plot(2,:),'LineWidth',LW4,'LineStyle',LineStyle_box)
        m_plot([lon_point(2),lon_point(2)],[lat_point(1),lat_point(2)],'color',color_plot(2,:),'LineWidth',LW4,'LineStyle',LineStyle_box)
    end
    ...
    if one == 3 || one >= 5 % tio
        ...
        m_plot([30,(30-b_ch2)/a_ch2],[30,30],'color',color_plot(3,:),'LineWidth',LW4,'LineStyle',LineStyle_box)
        m_plot([30,30],[-30,30],'color',color_plot(3,:),'LineWidth',LW4,'LineStyle',LineStyle_box)
        m_plot([30,(-30-b_ch2)/a_ch2],[-30,-30],'color',color_plot(3,:),'LineWidth',LW4,'LineStyle',LineStyle_box)
        m_plot([(-30-b_ch2)/a_ch2,(30-b_ch2)/a_ch2],[-30,30],'color',color_plot(3,:),'LineWidth',LW4,'LineStyle',LineStyle_box)
    end
    ...
    if one == 4 || one >= 5 % NTA
        ...
        m_plot([(20-b_ch)/a_ch,360],[30,30],'color',color_plot(4,:),'LineWidth',LW4,'LineStyle',LineStyle_box)
        m_plot([(20-b_ch)/a_ch,(20-b_ch)/a_ch],[20,30],'color',color_plot(4,:),'LineWidth',LW4,'LineStyle',LineStyle_box)
        m_plot([(20-b_ch)/a_ch,lon_point2],[20,lat_point2],'color',color_plot(4,:),'LineWidth',LW4,'LineStyle',LineStyle_box)
        m_plot([lon_point2,lon_point2],[-30,lat_point2],'color',color_plot(4,:),'LineWidth',LW4,'LineStyle',LineStyle_box)
        m_plot([lon_point2,360],[-30,-30],'color',color_plot(4,:),'LineWidth',LW4,'LineStyle',LineStyle_box)

        m_plot([0,20],[-30,-30],'color',color_plot(4,:),'LineWidth',LW4,'LineStyle',LineStyle_box)
        m_plot([20,20],[-30,10],'color',color_plot(4,:),'LineWidth',LW4,'LineStyle',LineStyle_box)
        m_plot([0,20],[10,10],'color',color_plot(4,:),'LineWidth',LW4,'LineStyle',LineStyle_box)
    end
    ...
    if one == 7
        m_text(lon_end,lat_end+ly_text,sixth_name2{six},'HorizontalAlignment','Right','FontName',FontName,'FontSize',FontSize2*four,'Interpreter','None');
    end
    %%
    ...
    if one == 1
        ...
        PaperPosition(4) = PaperPosition(4)+1;
        pathfig2 = [aimpath,fifth_name{2},'FigS',sixth_name3{six},first_name3{one},'_Com_',sixth_name{six},'Diff',second_name{two},'_Month',num2str(len_m(1)+2),'to',num2str(len_m(end)+2),'_mEnA_Exp_',first_name{one},'_Color',num2str(three)];
