...
one = 1;
first_name = {'Tropics_Turn'};
first_name2 = {'a'};
load(['F:\2023PMM_Work\bin_data\Mask_',first_name{one},'.mat']);
pathfig = [aimpath,'FigS5a_Mask_',first_name{one}];
mask_region(mask_region == 0) = nan; 
% mask_region(mask_region < 1) = 0.5; 

a = lat >= 45;
mask_region(:,a) = 1;

fig_lon = cat(1,lon,lon+360);
fig_mask_region = -cat(1,mask_region,mask_region);
%%
lon_start = 30;
lon_end = 360+30;
% lon_start = 180;
...
[cs,h] = m_contourf(fig_lon,lat,fig_mask_region','levelstep',0.5);
...
% m_text(lon_start,lat_end+5,['Exp_TPCtrl mask'],'FontName',FontName,'FontSize',FontSize,'Interpreter','None');
title(['CESM-TP'],'FontName',FontName,'FontSize',FontSize,'Interpreter','None');
