...
path_ctrl = ['F:\2023PMM_Work\Data_Ensamble\Exp_',first_name{1},'\'];
struct = dir([path_ctrl,'*.mat']);name_ctrl = {struct.name}';
path_g = ['F:\2023PMM_Work\Data_Ensamble\Exp_',first_name{6},'\'];
...
nino34_ssta_tpctrl = squeeze(mean(mean(data_ssta_tpctrl.ssta_ensamble(a,b,:),1),2));
...
nino4_ua_tpctrl = squeeze(mean(mean(data_taux_tpctrl.tauxa_ensamble(a,b,:),1),2))*100;
% g
a = data_ssta_g.lon >= lon_box_nino34(1) & data_ssta_g.lon <= lon_box_nino34(2);
b = data_ssta_g.lat >= -5 & data_ssta_g.lat <= 5;
nino34_ssta_g = squeeze(mean(mean(data_ssta_g.ssta_ensamble(a,b,:),1),2));
a = data_taux_g.lon >= lon_box_nino4(1) & data_taux_g.lon <= lon_box_nino4(2);
b = data_taux_g.lat >= -5 & data_taux_g.lat <= 5;
nino4_ua_g = squeeze(mean(mean(data_taux_g.tauxa_ensamble(a,b,:),1),2))*100;
...
ylabel('NINO4 ¦Ó_x','FontName',FontName,'FontSize',FontSize);% [¡Á10^{-3} N/m^{2}]
