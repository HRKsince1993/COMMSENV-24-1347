...
%         ,'NTAandTIOandPMMandSEPandWWBbmay','CESM_TP','NTAandTIOandPMMandSEPandWWBOctNov','TPCtrlNoWWB2'};
    ...
    path2 = ['F:\2023PMM_Work\Data_Ensamble\Exp_',second_name{two},'\Temp_Casely\'];
    ...
    savepath = [aimpath,'Compose_Sec',name1{1}(1:4),'Diff',second_name{two},name1{1}(5:end-7),'.mat']
    load([path1,name1{1}]);
    %%
    ...
    temp_ensamble = mean(bin_ssta,4);
    
    clear t_temp_ensamble p_temp_ensamble hh1 pp1 h1 p1
    for i1 = 1:size(bin_ssta,3)
        ...
        t_temp_ensamble(:,:,i1) = hh1;
        p_temp_ensamble(:,:,i1) = pp1;
    end
    %%
%     k = 1;
...
%     colorbar;
    %%
    save(savepath,'lon','lat','depth','date','temp_ensamble','readme','t_temp_ensamble','p_temp_ensamble');
    clear temp_ensamble readme t_temp_ensamble p_temp_ensamble
end