...
%         ,'NTAandTIOandPMMandSEPandWWBbmay','CESM_TP','NTAandTIOandPMMandSEPandWWBOctNov','TPCtrlNoWWB2'};
    ...
    i1 = 1;
    i2 = 1;
    readme = 'Meridional wind. Lon*Lat*Lev*Time. Velocity unit is m/s. Lev unit is level';
    %%
    for i1 = 1:length(name1)
        ...
        aimpath = ['J:\2023PMM_Work\Data_Ensamble\Exp_',first_name{one},'\WindV3_Casely\'];
        ...
        for i2 = 1:length(name2)
            bin_wind = double(ncread([path2,name2{i2}],'V'));
        ...
        vvel = uvel;
        %%
        datapath = [aimpath,'WindV3_Global_Monthly_',num2str(date(1,1)),'-',num2str(date(1,2),'%2.2i'),'_to_'...
            ,num2str(date(end,1)),'-',num2str(date(end,2),'%2.2i'),'_',name1{i1},'_CAM.mat'];
        save(datapath,'lon','lat','lev','vvel','date','readme');
        i1
    end
end