...
%         ,'NTAandTIOandPMMandSEPandWWBbmay','CESM_TP','NTAandTIOandPMMandSEPandWWBOctNov','TPCtrlNoWWB2'};
    ...
    mld_ensamble = mean(bin_sst,4);
    
    clear t_mld_ensamble p_mld_ensamble hh1 pp1 h1 p1
    for i1 = 1:size(bin_sst,3)
        ...
        t_mld_ensamble(:,:,i1) = hh1;
        p_mld_ensamble(:,:,i1) = pp1;
    end
    %%
%     k = 1;
%     contourf(lon,lat,mld_ensamble(:,:,k)');
%     colorbar;
    %%
    readme2 = name1;
    save(savepath,'lon','lat','date','mld_ensamble','readme','t_mld_ensamble','p_mld_ensamble','readme2');
    clear mld_ensamble readme t_mld_ensamble p_mld_ensamble
end
