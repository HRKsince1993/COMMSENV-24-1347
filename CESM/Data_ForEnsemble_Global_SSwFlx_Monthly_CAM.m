...
for one = 1:10 % 1:Ctrl;2:TPCtrl;3:PMM;4:SEP;5:TIOlg;6:NTAlg;7:NTAandTIOandPMMandSEPlg;8:NTAandTIOandPMMandSEPandWWBb;9:NTAandTIOandPMMandSEPandWWBbmay;
    ...
    i1 = 1;
    i2 = 1;
    readme = 'Net solar flux at surface,unit:W/m2';
    %%
...
%         ncdisp([path2,name2{1}])
        aimpath = ['F:\2023PMM_Work\Data_Ensemble\Exp_',first_name{one},'\SSwFlx_Casely\'];
        ...
        for i2 = 1:length(name2)
            bin_wind = double(ncread([path2,name2{i2}],'FSNS'));
        ...
        sswflx = uvel;
        %%
        datapath = [aimpath,'SSwFlx_Global_Monthly_',num2str(date(1,1)),'-',num2str(date(1,2),'%2.2i'),'_to_'...
            ,num2str(date(end,1)),'-',num2str(date(end,2),'%2.2i'),'_',name1{i1},'_CAM.mat'];
        save(datapath,'lon','lat','sswflx','date','readme');
        i1
    end
end