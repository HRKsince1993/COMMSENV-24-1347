...
%         ,'NTAandTIOandPMMandSEPandWWBbmay','CESM_TP','NTAandTIOandPMMandSEPandWWBOctNov','TPCtrlNoWWB2'};
    ...
    path1 = ['F:\2023PMM_Work\Data_Ensamble\Exp_',first_name{one},'\U10_Casely\'];
    path2 = ['F:\2023PMM_Work\Data_Ensamble\Exp_',second_name{two},'\U10_Casely\'];
    ...
    for i1 = 1:length(name1)
        ...
        bin_sst(:,:,:,i1) = data1.vel;
        bin_sst_tp(:,:,:,i1) = data2.vel;
    end
    bin_vela = bin_sst - bin_sst_tp;
    vela_ensamble = mean(bin_vela,4);
    
    clear t_vela_ensamble p_vela_ensamble hh1 pp1 h1 p1
    for i1 = 1:size(bin_vela,3)
        for i2 = 1:size(bin_vela,1)
            for i3 = 1:size(bin_vela,2)
                pro = bin_vela(i2,i3,i1,:);
                pro = pro(:);
        ...
        t_vela_ensamble(:,:,i1) = hh1;
        p_vela_ensamble(:,:,i1) = pp1;
    end
    %%
%     k = 1;
...
%     colorbar;
    %%
    save(savepath,'lon','lat','date','vela_ensamble','readme','t_vela_ensamble','p_vela_ensamble');
    clear vela_ensamble readme t_vela_ensamble p_vela_ensamble
end