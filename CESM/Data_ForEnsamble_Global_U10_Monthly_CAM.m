...
%         ncdisp([path2,name2{1}])
        aimpath = ['F:\2023PMM_Work\Data_Ensamble\Exp_',first_name{one},'\U10_Casely\'];
        ...
        for i2 = 1:length(name2)
            bin_wind = double(ncread([path2,name2{i2}],'U10'));%
        ...
        vel = uvel;
        %%
        datapath = [aimpath,'U10_Global_Monthly_',num2str(date(1,1)),'-',num2str(date(1,2),'%2.2i'),'_to_'...
            ,num2str(date(end,1)),'-',num2str(date(end,2),'%2.2i'),'_',name1{i1},'_CAM.mat'];
        save(datapath,'lon','lat','vel','date','readme');
        i1
    end
end