...
for one = 1:10 % 1:Ctrl;2:TPCtrl;3:PMM;4:SEP;5:TIOlg;6:NTAlg;7:NTAandTIOandPMMandSEPlg;8:NTAandTIOandPMMandSEPandWWBb;9:NTAandTIOandPMMandSEPandWWBbmay;
    ...
    z3_ensamble = mean(bin_sst,5);
    
%     clear t_z3_ensamble p_z3_ensamble hh1 pp1 h1 p1
...
%             end
%         end
%         t_z3_ensamble(:,:,:,i1) = hh1;
%         p_z3_ensamble(:,:,:,i1) = pp1;
%     end
    %%
    % k = 1;
    % contourf(lon,lat,z3_ensamble(:,:,k)','levelstep',2);
    % hold on
    % colorbar;
    % caxis([-5,35]);
    % hold off
    %%
    readme2 = name1;
%     save(savepath,'lon','lat','lev','date','z3_ensamble','t_z3_ensamble','p_z3_ensamble','readme','readme2');
    save(savepath,'lon','lat','lev','date','z3_ensamble','readme','readme2');
    clear z3_ensamble t_z3_ensamble p_z3_ensamble
end
