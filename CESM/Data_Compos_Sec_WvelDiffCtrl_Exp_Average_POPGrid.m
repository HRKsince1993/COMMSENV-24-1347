...
for one = 3:8 % 1:Ctrl;2:TPCtrl;3:PMM;4:SEP;5:TIOlg;6:NTAlg;7:NTAandTIOandPMMandSEPlg;8:NTAandTIOandPMMandSEPandWWBb;9:NTAandTIOandPMMandSEPandWWBbmay;
    ...
    savepath = [aimpath,'Compose_SecW',name1{1}(1:3),'Diff',second_name{two},name1{1}(4:end-7),'_Avr',num2str(l_mon(1)+2),'to',num2str(l_mon(end)+2),'Mon.mat']
    load([path1,name1{1}]);
    ...
    wvela_ensemble = mean(mean(bin_ssta(:,:,l_mon,:),4),3);
    ...
    clear t_wvela_ensemble p_wvela_ensemble hh1 pp1 h1 p1
        ...
        t_wvela_ensemble = hh1;
        p_wvela_ensemble = pp1;
    %%
%     k = 1;
%     contourf(lon,lat,wvela_ensemble(:,:,k)');
%     colorbar;
    %%
    save(savepath,'lon','lat','depth','date','wvela_ensemble','readme','t_wvela_ensemble','p_wvela_ensemble');
    clear wvela_ensemble readme t_wvela_ensemble p_wvela_ensemble
end