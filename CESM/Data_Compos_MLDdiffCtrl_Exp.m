...
for one = 1:10 % 1:Ctrl;2:TPCtrl;3:PMM;4:SEP;5:TIOlg;6:NTAlg;7:NTAandTIOandPMMandSEPlg;8:NTAandTIOandPMMandSEPandWWBb;9:NTAandTIOandPMMandSEPandWWBbmay;
    ...
    path1 = ['F:\2023PMM_Work\Data_Ensemble\Exp_',first_name{one},'\sMLD_Casely\'];
    path2 = ['F:\2023PMM_Work\Data_Ensemble\Exp_',second_name{two},'\sMLD_Casely\'];
    ...
    mlda_ensemble = mean(bin_ssta,4);
    
    clear t_mlda_ensemble p_mlda_ensemble hh1 pp1 h1 p1
    for i1 = 1:size(bin_ssta,3)
        ...
        t_mlda_ensemble(:,:,i1) = hh1;
        p_mlda_ensemble(:,:,i1) = pp1;
    end
    %%
%     k = 1;
%     contourf(lon,lat,mlda_ensemble(:,:,k)');
%     colorbar;
    %%
    save(savepath,'lon','lat','date','mlda_ensemble','readme','t_mlda_ensemble','p_mlda_ensemble');
    clear mlda_ensemble readme t_mlda_ensemble p_mlda_ensemble
end