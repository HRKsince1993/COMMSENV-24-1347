...
%         ,'NTAandTIOandPMMandSEPandWWBbmay','CESM_TP','NTAandTIOandPMMandSEPandWWBOctNov','TPCtrlNoWWB2'};
    ...
    path1 = ['F:\2023PMM_Work\Data_Ensamble\Exp_',first_name{one},'\sMLD_Casely\'];
    path2 = ['F:\2023PMM_Work\Data_Ensamble\Exp_',second_name{two},'\sMLD_Casely\'];
    ...
    for i1 = 1:length(name1)
        ...
        bin_sst(:,:,:,i1) = data1.mld;
        bin_sst_tp(:,:,:,i1) = data2.mld;
    end
    bin_ssta = bin_sst - bin_sst_tp;
    mlda_ensamble = mean(bin_ssta,4);
    
    clear t_mlda_ensamble p_mlda_ensamble hh1 pp1 h1 p1
    for i1 = 1:size(bin_ssta,3)
        ...
        t_mlda_ensamble(:,:,i1) = hh1;
        p_mlda_ensamble(:,:,i1) = pp1;
    end
    %%
%     k = 1;
%     contourf(lon,lat,mlda_ensamble(:,:,k)');
%     colorbar;
    %%
    save(savepath,'lon','lat','date','mlda_ensamble','readme','t_mlda_ensamble','p_mlda_ensamble');
    clear mlda_ensamble readme t_mlda_ensamble p_mlda_ensamble
end