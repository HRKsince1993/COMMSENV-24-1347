...
for one = 3:10 % 1:Ctrl;2:TPCtrl;3:PMM;4:SEP;5:TIOlg;6:NTAlg;7:NTAandTIOandPMMandSEPlg;8:NTAandTIOandPMMandSEPandWWBb;9:NTAandTIOandPMMandSEPandWWBbmay;
    ...
    path1 = ['J:\2023PMM_Work\Data_Ensamble\Exp_',first_name{one},'\WindV3_Casely\'];
    path2 = ['J:\2023PMM_Work\Data_Ensamble\Exp_',second_name{two},'\WindV3_Casely\'];
    ...
    savepath = [aimpath,'Compose_',name1{1}(1:6),'diff',second_name{two},'_Hadley',name1{1}(7:end-7),'.mat']
    load([path1,name1{1}]);
    readme = 'Meridional wind. Lat*Lev*Time. Velocity unit is m/s. Lev unit is level';
    %%
    clear bin_sst bin_sst_tp
    for i1 = 1:length(name1)
        data1 = load([path1,name1{i1}]);
        data2 = load([path2,name2{i1}]);
        bin_sst(:,:,:,i1) = squeeze(nanmean(data1.vvel,1));
        bin_sst_tp(:,:,:,i1) = squeeze(nanmean(data2.vvel,1));
    end
    bin_ssta = bin_sst - bin_sst_tp;
    vvela_ensamble = mean(bin_ssta,4);
    
    clear t_vvela_ensamble p_vvela_ensamble hh1 pp1 h1 p1
    for i1 = 1:size(bin_ssta,3)
        ...
        t_vvela_ensamble(:,:,i1) = hh1;
        p_vvela_ensamble(:,:,i1) = pp1;
    end
    %%
%     k = 1;
%     contourf(lon,lat,vvela_ensamble(:,:,k)');
%     colorbar;
    %%
    save(savepath,'lat','lev','date','vvela_ensamble','readme','t_vvela_ensamble','p_vvela_ensamble');
    clear vvela_ensamble readme t_vvela_ensamble p_vvela_ensamble
end