...
for one = 3:7 % 1:Ctrl;2:TPCtrl;3:PMM;4:SEP;5:TIOlg;6:NTAlg;7:NTAandTIOandPMMandSEPlg;8:NTAandTIOandPMMandSEPandWWBb;9:NTAandTIOandPMMandSEPandWWBbmay;
    ...
    path1 = ['F:\2023PMM_Work\Data_Ensamble\Exp_',first_name{one},'\SSHFlx_Casely\']
    path2 = ['F:\2023PMM_Work\Data_Ensamble\Exp_',second_name{two},'\SSHFlx_Casely\']
    ...
    for i1 = 1:length(name1)
        ...
        bin_sst(:,:,:,i1) = data1.ssflx;
        bin_sst_tp(:,:,:,i1) = data2.ssflx;
    end
    bin_ssta = bin_sst - bin_sst_tp;
    ssflxa_ensamble = mean(mean(bin_ssta(:,:,l_mon,:),4),3);
    ...
    clear t_slflxa_ensamble p_slflxa_ensamble hh1 pp1 h1 p1
        ...
        t_ssflxa_ensamble = hh1;
        p_ssflxa_ensamble = pp1;
    %%
%     k = 1;
%     contourf(lon,lat,slflxa_ensamble(:,:,k)');
%     colorbar;
    %%
    save(savepath,'lon','lat','date','ssflxa_ensamble','readme','t_ssflxa_ensamble','p_ssflxa_ensamble');
    clear ssflxa_ensamble readme t_ssflxa_ensamble p_ssflxa_ensamble
end