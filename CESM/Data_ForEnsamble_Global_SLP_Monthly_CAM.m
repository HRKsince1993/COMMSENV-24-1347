...
%         ,'NTAandTIOandPMMandSEPandWWBbmay','CESM_TP','NTAandTIOandPMMandSEPandWWBOctNov','TPCtrlNoWWB2'};
    ...
    for i1 = 1:length(name1)
        ...
        aimpath = ['F:\2023PMM_Work\Data_Ensamble\Exp_',first_name{one},'\SLP_Casely\'];
        ...
        for i2 = 1:length(name2)
            bin_wind = double(ncread([path2,name2{i2}],'PSL'))/100;% Pa to hPa
        ...
        slp = uvel;
        %%
        datapath = [aimpath,'SLP_Global_Monthly_',num2str(date(1,1)),'-',num2str(date(1,2),'%2.2i'),'_to_'...
            ,num2str(date(end,1)),'-',num2str(date(end,2),'%2.2i'),'_',name1{i1},'_CAM.mat'];
        save(datapath,'lon','lat','slp','date','readme');
        i1
    end
end