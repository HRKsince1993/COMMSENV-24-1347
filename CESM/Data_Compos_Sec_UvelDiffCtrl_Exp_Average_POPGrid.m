...
for one = 3:8 % 1:Ctrl;2:TPCtrl;3:PMM;4:SEP;5:TIOlg;6:NTAlg;7:NTAandTIOandPMMandSEPlg;8:NTAandTIOandPMMandSEPandWWBb;9:NTAandTIOandPMMandSEPandWWBbmay;
    ...
    path1 = ['F:\2023PMM_Work\Data_Ensemble\Exp_',first_name{one},'\OcnVel_Casely\'];
    path2 = ['F:\2023PMM_Work\Data_Ensemble\Exp_',second_name{two},'\OcnVel_Casely\'];
    ...
    savepath = [aimpath,'Compose_SecU',name1{1}(1:3),'Diff',second_name{two},name1{1}(4:end-7),'_Avr',num2str(l_mon(1)+2),'to',num2str(l_mon(end)+2),'Mon.mat']
    load([path1,name1{1}]);
    ...
    for i1 = 1:length(name1)
        ...
        bin_sst(:,:,:,i1) = data1.sec_uvel;
        bin_sst_tp(:,:,:,i1) = data2.sec_uvel;
    end
    bin_ssta = bin_sst - bin_sst_tp;
    uvela_ensemble = mean(mean(bin_ssta(:,:,l_mon,:),4),3);
    ...
    clear t_uvela_ensemble p_uvela_ensemble hh1 pp1 h1 p1
        ...
        t_uvela_ensemble = hh1;
        p_uvela_ensemble = pp1;
    %%
%     k = 1;
%     contourf(lon,lat,uvela_ensemble(:,:,k)');
%     colorbar;
    %%
    save(savepath,'lon','lat','depth','date','uvela_ensemble','readme','t_uvela_ensemble','p_uvela_ensemble');
    clear uvela_ensemble readme t_uvela_ensemble p_uvela_ensemble
end