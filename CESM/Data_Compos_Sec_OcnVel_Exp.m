...
for one = 1:10 % 1:Ctrl;2:TPCtrl;3:PMM;4:SEP;5:TIOlg;6:NTAlg;7:NTAandTIOandPMMandSEPlg;8:NTAandTIOandPMMandSEPandWWBb;9:NTAandTIOandPMMandSEPandWWBbmay;
    ...
    sec_uvel_ensemble = mean(bin_uvel,4);
    sec_wvel_ensemble = mean(bin_wvel,4);
    %%
    % k = 1;
    % contourf(lon,depth,sec_uvel_ensemble(:,:,k)','levelstep',0.1);
    % hold on
    % colorbar;
    % caxis([-1,1]);
    % hold off
    %%
    readme2 = name1;
    save(savepath,'lon','lat','depth','date','sec_uvel_ensemble','sec_wvel_ensemble','readme','readme2');
end