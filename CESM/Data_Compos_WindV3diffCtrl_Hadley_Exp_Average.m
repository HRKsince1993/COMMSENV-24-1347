...
%         ,'NTAandTIOandPMMandSEPandWWBbmay','CESM_TP','NTAandTIOandPMMandSEPandWWBOctNov','TPCtrlNoWWB2'};
    ...
    for i1 = 1:length(name1)
        ...
        bin_sst(:,:,:,i1) = squeeze(nanmean(data1.vvel(lon_l,:,:,:),1));
        bin_sst_tp(:,:,:,i1) = squeeze(nanmean(data2.vvel(lon_l,:,:,:),1));
    end
    bin_ssta = bin_sst - bin_sst_tp;
    %%
    vvela_ensamble = mean(mean(bin_ssta(:,:,l_mon,:),4),3);
    ...
    clear t_vvela_ensamble p_vvela_ensamble hh1 pp1 h1 p1
        ...
        t_vvela_ensamble = hh1;
        p_vvela_ensamble = pp1;
    %%
%     k = 1;
