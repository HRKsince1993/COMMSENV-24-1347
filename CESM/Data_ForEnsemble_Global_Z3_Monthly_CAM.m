...
for one = 1:10 % 1:Ctrl;2:TPCtrl;3:PMM;4:SEP;5:TIOlg;6:NTAlg;7:NTAandTIOandPMMandSEPlg;8:NTAandTIOandPMMandSEPandWWBb;9:NTAandTIOandPMMandSEPandWWBbmay;
    ...
    i1 = 1;
    i2 = 1;
    readme = 'lon*lat*height*time. Geopotential Height (above sea level). Height unit is m. lev unit is level';
    %%
    for i1 = 1:length(name1)
        ...
        aimpath = ['J:\2023PMM_Work\Data_Ensemble\Exp_',first_name{one},'\Z3_Casely\'];
        ...
        for i2 = 1:length(name2)
            bin_wind = double(ncread([path2,name2{i2}],'Z3'));
        ...
        z3 = uvel;
        %%
        datapath = [aimpath,'Z3_Global_Monthly_',num2str(date(1,1)),'-',num2str(date(1,2),'%2.2i'),'_to_'...
            ,num2str(date(end,1)),'-',num2str(date(end,2),'%2.2i'),'_',name1{i1},'_CAM.mat'];
        save(datapath,'lon','lat','lev','z3','date','readme');
        i1
    end
end