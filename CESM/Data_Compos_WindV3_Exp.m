...
%         ,'NTAandTIOandPMMandSEPandWWBbmay','CESM_TP','NTAandTIOandPMMandSEPandWWBOctNov','TPCtrlNoWWB2'};
    ...
    for i1 = 1:length(name1)
        ...
        bin_sst(:,:,:,:,i1) = data1.vvel;
    end
    vvel_ensamble = mean(bin_sst,5);
    
%     clear t_vvel_ensamble p_vvel_ensamble hh1 pp1 h1 p1
...
%             end
%         end
%         t_vvel_ensamble(:,:,:,i1) = hh1;
%         p_vvel_ensamble(:,:,:,i1) = pp1;
%     end
    %%
    % k = 1;
    % contourf(lon,lat,vvel_ensamble(:,:,k)','levelstep',2);
    % hold on
    % colorbar;
    % caxis([-5,35]);
    % hold off
    %%
    readme2 = name1;
%     save(savepath,'lon','lat','lev','date','vvel_ensamble','t_vvel_ensamble','p_vvel_ensamble','readme','readme2');
    save(savepath,'lon','lat','lev','date','vvel_ensamble','readme','readme2');
    clear vvel_ensamble t_vvel_ensamble p_vvel_ensamble
end
