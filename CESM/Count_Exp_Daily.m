...
for one = 41:41;% 1;Ctrl;2:NTA;3:PMM;4:TIO;5:TroPac:6£ºTPCtrl;7:NTA15;8:NTA30;9:AtlNino;10:NoWNP;11:NoPMM;12:NTAandTIO;13:TroPacNTA;14:PMMandNTA;15:TPCtrlG;16:SEP;
    ...
    i2 = 1;
    for i1 = 1:11 % Case
        path2a = ['H:\CESM\Output_Data\HRK_2023ENSO_Ensemble\Exp_',first_name{one}...
            ,'\Exp_',first_name{one},'_',second_name{two},'_',num2str(i1-1,'%2.2i'),'\ocean_daily\'];
        ...
        for i2 = 1:length(name2a)
            if i2 <= 10
                a1 = ['test_HRK_',first_name4,first_name3,'case',first_name2,second_name{two},'_',num2str(k,'%2.2i')...
                    ,'_code_',num2str(i1-k),'.pop.h.nday.2023-',num2str(i2+2,'%2.2i'),'-01.nc'];
            else
                a1 = ['test_HRK_',first_name4,first_name3,'case',first_name2,second_name{two},'_',num2str(k,'%2.2i')...
                    ,'_code_',num2str(i1-k),'.pop.h.nday.2024-',num2str(i2-10,'%2.2i'),'-01.nc'];
            end
            a2 = name2a{i2};
    ...
    ['one=',num2str(one),', check daily ',first_name{one},' done']
end