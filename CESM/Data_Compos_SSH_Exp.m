...
for one = 1:10 % 1:Ctrl;2:TPCtrl;3:PMM;4:SEP;5:TIOlg;6:NTAlg;7:NTAandTIOandPMMandSEPlg;8:NTAandTIOandPMMandSEPandWWBb;9:NTAandTIOandPMMandSEPandWWBbmay;
    ...
    ssh_ensemble = mean(bin_ssh,4);
    
    clear t_ssh_ensemble p_ssh_ensemble hh1 pp1 h1 p1
    for i1 = 1:size(bin_ssh,3)
        ...
        t_ssh_ensemble(:,:,i1) = hh1;
        p_ssh_ensemble(:,:,i1) = pp1;
    end
    %%
    % k = 1;
    % contourf(lon,lat,ssh_ensemble(:,:,k)','levelstep',2);
    % hold on
    % colorbar;
    % caxis([-5,35]);
    % hold off
    %%
    readme2 = name1;
    save(savepath,'lon','lat','date','ssh_ensemble','t_ssh_ensemble','p_ssh_ensemble','readme2');
