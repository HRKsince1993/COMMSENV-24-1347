...
for one = 1:8 % 1:Ctrl;2:TPCtrl;3:PMM;4:SEP;5:TIOlg;6:NTAlg;7:NTAandTIOandPMMandSEPlg;8:NTAandTIOandPMMandSEPandWWBb;9:NTAandTIOandPMMandSEPandWWBbmay;
    ...
    path1 = ['F:\2023PMM_Work\Data_Ensamble\Exp_',first_name{one},'\TauyA_Casely\'];
    ...
    for i1 = 1:length(name1)
        ...
        bin_ssta(:,:,:,i1) = data1.tauya;
    end
    tauya_ensamble = mean(mean(bin_ssta(:,:,l_mon,:),4),3);
    bin2_ssta = squeeze(mean(bin_ssta(:,:,l_mon,:),3));
    %%
    clear t_tauya_ensamble p_tauya_ensamble hh1 pp1 h1 p1
        ...
        t_tauya_ensamble = hh1;
        p_tauya_ensamble = pp1;
    %%
    % k = 1;
    ...
    %%
    readme2 = name1;
    save(savepath,'lon','lat','date','tauya_ensamble','t_tauya_ensamble','p_tauya_ensamble','readme2');
    % save(savepath,'lon','lat','date','ssta_ensamble');
    clear tauya_ensamble t_tauya_ensamble p_tauya_ensamble
end