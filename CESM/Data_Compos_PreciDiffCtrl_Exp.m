...
for one = 1:10 % 1:Ctrl;2:TPCtrl;3:PMM;4:SEP;5:TIOlg;6:NTAlg;7:NTAandTIOandPMMandSEPlg;8:NTAandTIOandPMMandSEPandWWBb;9:NTAandTIOandPMMandSEPandWWBbmay;
    ...
    path1 = ['F:\2023PMM_Work\Data_Ensemble\Exp_',first_name{one},'\Prect_Casely\'];
    path2 = ['F:\2023PMM_Work\Data_Ensemble\Exp_',second_name{two},'\Prect_Casely\'];
    ...
    savepath = [aimpath,'Compose_',name1{1}(1:5),'diff',second_name{two},name1{1}(6:end-7),'.mat']
    load([path1,name1{1}]);
    ...
    precia_ensemble = mean(bin_ssta,4);
    
    clear t_precia_ensemble p_precia_ensemble hh1 pp1 h1 p1
    for i1 = 1:size(bin_ssta,3)
        ...
        t_precia_ensemble(:,:,i1) = hh1;
        p_precia_ensemble(:,:,i1) = pp1;
    end
    %%
%     k = 1;
%     contourf(lon,lat,precia_ensemble(:,:,k)');
%     colorbar;
    %%
    save(savepath,'lon','lat','date','precia_ensemble','readme','t_precia_ensemble','p_precia_ensemble');
    clear precia_ensemble readme t_precia_ensemble p_precia_ensemble
end