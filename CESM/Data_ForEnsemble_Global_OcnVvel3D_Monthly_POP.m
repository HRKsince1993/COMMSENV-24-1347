...
%         ncdisp([path2,name2{1}])
        aimpath2 = ['F:\2023PMM_Work\Data_Ensemble\Exp_',first_name{one},'\OcnVvel3D_Global_Casely\'];
        ...
        for i2 = 1:length(name2)
            ...
            bin_vvel = ncread([path2,name2{i2}],'VVEL',[1,1,1,1],[inf,inf,16,1])/100;% Ctrl, cm/s to m/s
%             bin_temp(bin_temp == 0) = nan;

            clear bin_vvel2
            for i3 = 1:size(bin_vvel,3)
                bin_vvel2(:,:,i3) = griddata(tlon,tlat,bin_vvel(:,:,i3),lon_grid,lat_grid,'linear')';
            end
        ...
        %%
        readme = 'unit is m/s';
        datapath2 = [aimpath2,'OcnVvel3D_Global_Monthly_',num2str(date(1,1)),'-',num2str(date(1,2),'%2.2i'),'_to_'...
