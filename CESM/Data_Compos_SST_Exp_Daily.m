...
for one = 1:10 % 1:Ctrl;2:TPCtrl;3:PMM;4:SEP;5:TIOlg;6:NTAlg;7:NTAandTIOandPMMandSEPlg;8:NTAandTIOandPMMandSEPandWWBb;9:NTAandTIOandPMMandSEPandWWBbmay;
    ...
    path1 = ['F:\2023PMM_Work\Data_Ensemble\Exp_',first_name{one},'\SST_Casely_Daily\'];
    ...
    sst_ensemble = bin_sst/length(name1);

    % clear t_sst_ensemble p_sst_ensemble hh1 pp1 h1 p1
    ...
    %         end
    %     end
    %     t_sst_ensemble(:,:,i1) = hh1;
    %     p_sst_ensemble(:,:,i1) = pp1;
    % end
    %%
    % k = 1;
    ...
    %%
    readme2 = name1;
    % save(savepath,'lon','lat','date','sst_ensemble','t_sst_ensemble','p_sst_ensemble','readme2');
    save(savepath,'lon','lat','date','sst_ensemble','readme','time','readme2');
    clear sst_ensemble t_sst_ensemble p_sst_ensemble
end