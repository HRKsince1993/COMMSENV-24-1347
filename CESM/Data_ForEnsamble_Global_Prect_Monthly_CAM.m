...
for one = 1:10 % 1:Ctrl;2:TPCtrl;3:PMM;4:SEP;5:TIOlg;6:NTAlg;7:NTAandTIOandPMMandSEPlg;8:NTAandTIOandPMMandSEPandWWBb;9:NTAandTIOandPMMandSEPandWWBbmay;
    ...
    for i1 = 1:length(name1)
        ...
        aimpath = ['F:\2023PMM_Work\Data_Ensamble\Exp_',first_name{one},'\Prect_Casely\'];
        ...
        for i2 = 1:length(name2)
            bin_windu = double(ncread([path2,name2{i2}],'PRECC'));
            bin_windv = double(ncread([path2,name2{i2}],'PRECL'));
            bin_wind = bin_windu + bin_windv;
        ...
        prect = uvel;
        %%
        datapath = [aimpath,'Preci_Global_Monthly_',num2str(date(1,1)),'-',num2str(date(1,2),'%2.2i'),'_to_'...
            ,num2str(date(end,1)),'-',num2str(date(end,2),'%2.2i'),'_',name1{i1},'_CAM.mat'];
        save(datapath,'lon','lat','prect','date','readme');
        i1
    end
end