...
for one = 1:10 % 1:Ctrl;2:TPCtrl;3:PMM;4:SEP;5:TIOlg;6:NTAlg;7:NTAandTIOandPMMandSEPlg;8:NTAandTIOandPMMandSEPandWWBb;9:NTAandTIOandPMMandSEPandWWBbmay;
    ...
    taux_ensamble = mean(bin_sst,4);
    
    clear t_taux_ensamble p_taux_ensamble hh1 pp1 h1 p1
    ...
    %%
    % k = 1;
    % contourf(lon,lat,taux_ensamble(:,:,k)','levelstep',2);
    % hold on
    % colorbar;
    % caxis([-5,35]);
    % hold off
    %%
    readme2 = name1;
    save(savepath,'lon','lat','date','taux_ensamble','t_taux_ensamble','p_taux_ensamble','readme2');
