...
for one = 2:10 % 1:Ctrl;2:TPCtrl;3:PMM;4:SEP;5:TIOlg;6:NTAlg;7:NTAandTIOandPMMandSEPlg;8:NTAandTIOandPMMandSEPandWWBb;9:NTAandTIOandPMMandSEPandWWBbmay;
    ...
    slp_ensemble = mean(bin_sst,4);
    
    clear t_slp_ensemble p_slp_ensemble hh1 pp1 h1 p1
    for i1 = 1:size(bin_sst,3)
        ...
        t_slp_ensemble(:,:,i1) = hh1;
        p_slp_ensemble(:,:,i1) = pp1;
    end
    %%
%     k = 1;
%     contourf(lon,lat,slp_ensemble(:,:,k)');
%     colorbar;
    %%
    readme2 = name1;
    save(savepath,'lon','lat','date','slp_ensemble','readme','t_slp_ensemble','p_slp_ensemble','readme2');
    clear slp_ensemble readme t_slp_ensemble p_slp_ensemble
end
