...
% data4 = load([path1,'Compose_Wind_diff',second_name{two},'_Global_Monthly_2023-03_to_2024-02_Exp_',first_name{one},name1{1}(end-12:end-5),'_.mat']);% WindDiff

readme = data.readme;
date = data.date;
ssta = data.ssta_ensamble;
t_ssta = data.t_ssta_ensamble;

switch one
    case 2 % SETP
        path1 = [360-100,0];
        path2 = [360-80,-20];
        datapath = [aimpath,'Compose_zHovPath_SSTdiff',second_name{two},name1{1}(21:end-7),'.mat']
end
...
for i1 = 1:length(cyc_lon)
    ...
    bin(i1,:) = squeeze(mean(ssta(bin_lon(i1),b2,:),2));
end

clear bin2
for i1 = 1:length(cyc_lon)
    b2 = lat >= cyc_lat(i1)-len & lat <= cyc_lat(i1)+len;
    bin2(i1,:) = squeeze(mean(t_ssta(bin_lon(i1),b2,:),2));
end

hpath_ssta = bin;
hpath_ssta_t = bin2;
hpath_ssta_t( hpath_ssta_t>0 ) = 1;
hpath_ssta_t(hpath_ssta_t == 0) = nan;
%%
contourf(hpath_ssta','levelstep',0.1);
colorbar;
caxis([-1.5,1.5]);
% colormap(b2r(-1,1));
%%
save(datapath,'hpath_ssta','hpath_ssta_t','cyc_lon','cyc_lat','date','readme');
