...
for one = 1:10 % 1:Ctrl;2:TPCtrl;3:PMM;4:SEP;5:TIOlg;6:NTAlg;7:NTAandTIOandPMMandSEPlg;8:NTAandTIOandPMMandSEPandWWBb;9:NTAandTIOandPMMandSEPandWWBbmay;
    ...
    tauy_ensamble = mean(bin_sst,4);
    
    clear t_tauy_ensamble p_tauy_ensamble hh1 pp1 h1 p1
    for i1 = 1:size(bin_sst,3)
        ...
        t_tauy_ensamble(:,:,i1) = hh1;
        p_tauy_ensamble(:,:,i1) = pp1;
    end
    %%
    % k = 1;
    % contourf(lon,lat,tauy_ensamble(:,:,k)','levelstep',2);
    % hold on
    % colorbar;
    % caxis([-5,35]);
    % hold off
    %%
    readme2 = name1;
    save(savepath,'lon','lat','date','tauy_ensamble','t_tauy_ensamble','p_tauy_ensamble','readme2');
