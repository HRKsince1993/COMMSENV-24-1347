...
%         ,'NTAandTIOandPMMandSEPandWWBbmay','CESM_TP','NTAandTIOandPMMandSEPandWWBOctNov','TPCtrlNoWWB2'};
    ...
    path1 = ['F:\2023PMM_Work\Data_Ensamble\Exp_',first_name{one},'\SLwFlx_Casely\']
    path2 = ['F:\2023PMM_Work\Data_Ensamble\Exp_',second_name{two},'\SLwFlx_Casely\']
    ...
    savepath = [aimpath,'Compose_',name1{1}(1:6),'Diff',second_name{two},name1{1}(7:end-7),'Avr',num2str(l_mon(1)+2),'to',num2str(l_mon(end)+2),'Mon.mat']
    load([path1,name1{1}]);
    %%
    ...
    for i1 = 1:length(name1)
        ...
        bin_sst(:,:,:,i1) = data1.slwflx;
        bin_sst_tp(:,:,:,i1) = data2.slwflx;
    end
    bin_ssta = bin_sst - bin_sst_tp;
    slwflxa_ensamble = mean(mean(bin_ssta(:,:,l_mon,:),4),3);
    ...
    clear t_slflxa_ensamble p_slflxa_ensamble hh1 pp1 h1 p1
        ...
        t_slwflxa_ensamble = hh1;
        p_slwflxa_ensamble = pp1;
    %%
%     k = 1;
%     contourf(lon,lat,slflxa_ensamble(:,:,k)');
%     colorbar;
    %%
    save(savepath,'lon','lat','date','slwflxa_ensamble','readme','t_slwflxa_ensamble','p_slwflxa_ensamble');
    clear slwflxa_ensamble readme t_slwflxa_ensamble p_slwflxa_ensamble
end