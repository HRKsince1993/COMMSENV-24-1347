...
%%
date = nan(length(name)*12,2);
sec_ptmp = nan(length(sec_lon),length(sec_depth),length(name)*12);
for i1 = 1:length(name)
    ...
    sec_ptmp(:,:,(i1-1)*12+1:i1*12) = squeeze(nanmean(bin,2));
    
    bin_time = double( ncread([path,name{i1}],'time'));
    [bin_time(:,1),bin_time(:,2),~,~,~,~] = mjd19502date(bin_time - bnd_time);
    date((i1-1)*12+1:i1*12,:) = bin_time;
end
a = abs(sec_ptmp) >= 1e4;
sec_ptmp(a) = nan;
%%
datapath = [aimpath,'Sec',first_name{one},'_PTmp_',num2str(date(1,1)),'to',num2str(date(end,1)),'.mat'];
